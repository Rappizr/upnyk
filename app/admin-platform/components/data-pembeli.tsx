  ...
  const muatDataPembeli = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase
        .from("pembeli")
        .select("id, profile_id, nama, alamat, no_hp, email, status, created_at")
        .order("created_at", { ascending: false });

      if (error) {
        console.error("Gagal membaca data pembeli:", error.message || JSON.stringify(error));
        setLoading(false);
        return;
      }
      ...
      const mapped: Pembeli[] = (data || []).map((p: any) => ({
        ...
        email: p.email || "-",
        telepon: p.no_hp || "-",
        alamat: p.alamat || "Alamat belum diisi",
        status: p.status === "suspended" || p.status === "nonaktif" ? "Nonaktif" : "Aktif",
      }));

      setPembeliList(mapped);
    } catch (err) {
      console.error("Error muatDataPembeli:", err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <main style={{ padding: "1.25rem clamp(1rem, 4vw, 1.75rem)", fontFamily: "sans-serif" }}>
      ...
      <div className="search-wrapper-mobile" style={{ position: "relative", marginBottom: "1.5rem", maxWidth: "420px" }}>
        ...
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari nama atau email pembeli..." style={{ width: "100%", padding: "0.5rem 1rem 0.5rem 2.25rem", borderRadius: "8px", border: "1px solid #CBD5E1", fontSize: "0.9rem", outline: "none", boxSizing: "border-box" }} />
      </div>
      ...
      <div className="buyer-table-container" style={{ background: "white", borderRadius: "12px", border: "1px solid #E2E8F0", overflow: "hidden" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", fontSize: "0.9rem", minWidth: "650px" }}>
            ...
            <tbody>
              {filtered.length === 0 && (
                <tr><td colSpan={5} style={{ padding: "2rem", textAlign: "center", color: "#94A3B8" }}>Tidak ada data pembeli yang cocok.</td></tr>
              )}
              {filtered.map((b) => (
      ...
      {detail && (
        <div onClick={() => setDetail(null)} style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "1rem" }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "white", borderRadius: "14px", padding: "1.5rem", width: "400px", maxWidth: "100%" }}>
            ...
            <p style={{ margin: "0 0 1.1rem 0", fontSize: "0.8rem", color: "#94A3B8" }}>#{detail.id.slice(0, 8).toUpperCase()}</p>
