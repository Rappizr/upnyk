...
interface KategoriItem {
  ...
  anggota: string[];
  pertumbuhan: number; // persen, bisa negatif
}

const initialData: KategoriItem[] = [
  { id: "CAT-A01", nama: "Makanan & Minuman", totalItem: "1.240 UMKM", status: "Aktif", pertumbuhan: 8, anggota: ["Keripik Tempe Sanan", "Sambal Bu Rudy", "Kopi Arabika Gayo"] },
  { id: "CAT-A02", nama: "Kerajinan & Kriya", totalItem: "450 UMKM", status: "Aktif", pertumbuhan: 3, anggota: ["Kerajinan Bambu Jaya", "Anyaman Rotan Kalimantan"] },
...
export default function KategoriUsahaPage() {
  ...
  function handleAddSubmit(e: FormEvent) {
    ...
    const id = `CAT-A${String(nextNumber).padStart(2, "0")}`;
    setKategoriList((prev) => [{ id, nama: addForm.nama, totalItem: addForm.totalItem || "0 UMKM", status: addForm.status, pertumbuhan: 0, anggota: [] }, ...prev]);
  ...
  return (
    <main style={{ flex: 1, padding: "2rem", overflowY: "auto", height: "100vh", width: "100%" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem", flexWrap: "wrap", gap: "1rem" }}>
        <div>
          <h1 style={{ margin: 0, fontSize: "1.75rem", fontWeight: 700, color: "#1E293B" }}>Kategori Usaha</h1>
          <p style={{ margin: "0.25rem 0 0 0", color: "#64748B", fontSize: "0.95rem" }}>Kelola segmentasi sektor bisnis dan awasi pertumbuhan tiap kategori usaha UMKM.</p>
        </div>
        <button onClick={() => setShowAddModal(true)} style={{ background: "#2563EB", color: "white", border: "none", padding: "0.625rem 1.25rem", borderRadius: "6px", fontWeight: 600, cursor: "pointer", fontSize: "0.9rem" }}>+ Tambah Kategori Usaha</button>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "1rem", marginBottom: "1.5rem" }}>
        ...
        <div style={{ background: "white", padding: "1.1rem", borderRadius: "12px", border: "1px solid #E2E8F0", display: "flex", alignItems: "center", gap: "0.9rem" }}>
          ...
          <div><div style={{ fontSize: "1.4rem", fontWeight: 700, color: "#1E293B" }}>{totalAnggota}</div><div style={{ fontSize: "0.8rem", color: "#64748B" }}>Total UMKM Terpantau</div></div>
        </div>
      </div>
      ...
      {detailItem && (
        <div onClick={() => setDetailItem(null)} style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "1rem" }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "white", borderRadius: "14px", padding: "1.5rem", width: "480px", maxWidth: "100%", maxHeight: "85vh", overflowY: "auto" }}>
            ...
            <div style={{ fontSize: "0.85rem", fontWeight: 700, color: "#1E293B", marginBottom: "0.5rem" }}>UMKM dalam kategori ini ({detailItem.anggota.length})</div>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
              {detailItem.anggota.length === 0 && <p style={{ fontSize: "0.85rem", color: "#94A3B8" }}>Belum ada UMKM terhubung ke kategori ini.</p>}
      ...
      {showAddModal && (
        <div onClick={() => setShowAddModal(false)} style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "1rem" }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "white", borderRadius: "12px", padding: "1.5rem", width: "400px", maxWidth: "100%" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.1rem" }}>
              <h2 style={{ margin: 0, fontSize: "1.1rem", fontWeight: 700, color: "#1E293B" }}>Tambah Kategori Usaha</h2>
            ...
            <form onSubmit={handleAddSubmit} style={{ display: "flex", flexDirection: "column", gap: "0.9rem" }}>
              <div>
                <label style={{ display: "block", fontSize: "0.8rem", fontWeight: 600, color: "#334155", marginBottom: "0.3rem" }}>Nama Kategori *</label>
                <input required value={addForm.nama} onChange={(e) => setAddForm({ ...addForm, nama: e.target.value })} placeholder="Contoh: Produk Herbal & Jamu" style={{ width: "100%", padding: "0.55rem 0.75rem", borderRadius: "6px", border: "1px solid #CBD5E1", fontSize: "0.9rem", outline: "none" }} />
              </div>
              <div>
                <label style={{ display: "block", fontSize: "0.8rem", fontWeight: 600, color: "#334155", marginBottom: "0.3rem" }}>Total Terhubung (opsional)</label>
                <input value={addForm.totalItem} onChange={(e) => setAddForm({ ...addForm, totalItem: e.target.value })} placeholder="Contoh: 0 UMKM" style={{ width: "100%", padding: "0.55rem 0.75rem", borderRadius: "6px", border: "1px solid #CBD5E1", fontSize: "0.9rem", outline: "none" }} />
              </div>
              <div style={{ display: "flex", gap: "0.5rem", marginTop: "0.5rem" }}>
