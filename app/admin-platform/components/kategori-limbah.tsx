...
const initialData: KategoriItem[] = [
  { id: "CAT-D01", nama: "Ampas Kopi Organik", totalItem: "12 Industri", status: "Aktif", pertumbuhan: 15, anggota: ["Kopi Arabika Gayo", "Kedai Kopi Kita"] },
  { id: "CAT-D02", nama: "Sekam Padi & Kulit Kayu", totalItem: "34 Industri", status: "Aktif", pertumbuhan: 6, anggota: ["Penggilingan Padi Makmur", "Sawmill Kalimantan"] },
...
export default function KategoriLimbahPage() {
  ...
  function handleAddSubmit(e: FormEvent) {
    ...
    const id = `CAT-D${String(nextNumber).padStart(2, "0")}`;
    setKategoriList((prev) => [{ id, nama: addForm.nama, totalItem: addForm.totalItem || "0 Industri", status: addForm.status, pertumbuhan: 0, anggota: [] }, ...prev]);
  ...
  return (
    <main style={{ flex: 1, padding: "2rem", overflowY: "auto", height: "100vh", width: "100%" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem", flexWrap: "wrap", gap: "1rem" }}>
        <div>
          <h1 style={{ margin: 0, fontSize: "1.75rem", fontWeight: 700, color: "#1E293B" }}>Kategori Limbah</h1>
          <p style={{ margin: "0.25rem 0 0 0", color: "#64748B", fontSize: "0.95rem" }}>Kelola pengelompokan limbah sisa produksi dan awasi industri penghasilnya untuk ekonomi sirkular.</p>
        </div>
        <button onClick={() => setShowAddModal(true)} style={{ background: "#2563EB", color: "white", border: "none", padding: "0.625rem 1.25rem", borderRadius: "6px", fontWeight: 600, cursor: "pointer", fontSize: "0.9rem" }}>+ Tambah Kategori Limbah</button>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "1rem", marginBottom: "1.5rem" }}>
        ...
        <div style={{ background: "white", padding: "1.1rem", borderRadius: "12px", border: "1px solid #E2E8F0", display: "flex", alignItems: "center", gap: "0.9rem" }}>
          ...
          <div><div style={{ fontSize: "1.4rem", fontWeight: 700, color: "#1E293B" }}>{totalAnggota}</div><div style={{ fontSize: "0.8rem", color: "#64748B" }}>Total Industri Terpantau</div></div>
        </div>
      </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", fontSize: "0.9rem", minWidth: "650px" }}>
          <thead>
            <tr style={{ background: "#F8FAFC", borderBottom: "1px solid #E2E8F0" }}>
              ...
              <th style={{ padding: "1rem", color: "#475569" }}>Total Industri</th>
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ background: "white", borderRadius: "14px", padding: "1.5rem", width: "480px", maxWidth: "100%", maxHeight: "85vh", overflowY: "auto" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem", marginBottom: "1.25rem" }}>
              <div style={{ background: "#F8FAFC", borderRadius: "10px", padding: "0.75rem" }}>
                <div style={{ fontSize: "0.7rem", color: "#64748B", textTransform: "uppercase", fontWeight: 700 }}>Total Industri</div>
            ...
            <div style={{ fontSize: "0.85rem", fontWeight: 700, color: "#1E293B", marginBottom: "0.5rem" }}>Industri penghasil limbah ini ({detailItem.anggota.length})</div>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
              {detailItem.anggota.length === 0 && <p style={{ fontSize: "0.85rem", color: "#94A3B8" }}>Belum ada industri terhubung ke kategori ini.</p>}
        ...
        <div onClick={() => setEditItem(null)} style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "1rem" }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "white", borderRadius: "12px", padding: "1.5rem", width: "400px", maxWidth: "100%" }}>
            ...
            <form onSubmit={handleEditSubmit} style={{ display: "flex", flexDirection: "column", gap: "0.9rem" }}>
              ...
              <div>
                <label style={{ display: "block", fontSize: "0.8rem", fontWeight: 600, color: "#334155", marginBottom: "0.3rem" }}>Total Industri</label>
      ...
      {showAddModal && (
        <div onClick={() => setShowAddModal(false)} style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "1rem" }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "white", borderRadius: "12px", padding: "1.5rem", width: "400px", maxWidth: "100%" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.1rem" }}>
              <h2 style={{ margin: 0, fontSize: "1.1rem", fontWeight: 700, color: "#1E293B" }}>Tambah Kategori Limbah</h2>
            ...
            <form onSubmit={handleAddSubmit} style={{ display: "flex", flexDirection: "column", gap: "0.9rem" }}>
              <div>
                <label style={{ display: "block", fontSize: "0.8rem", fontWeight: 600, color: "#334155", marginBottom: "0.3rem" }}>Nama Kategori *</label>
                <input required value={addForm.nama} onChange={(e) => setAddForm({ ...addForm, nama: e.target.value })} placeholder="Contoh: Limbah Cangkang Kelapa Sawit" style={{ width: "100%", padding: "0.55rem 0.75rem", borderRadius: "6px", border: "1px solid #CBD5E1", fontSize: "0.9rem", outline: "none" }} />
              </div>
              <div>
                <label style={{ display: "block", fontSize: "0.8rem", fontWeight: 600, color: "#334155", marginBottom: "0.3rem" }}>Total Industri (opsional)</label>
                <input value={addForm.totalItem} onChange={(e) => setAddForm({ ...addForm, totalItem: e.target.value })} placeholder="Contoh: 0 Industri" style={{ width: "100%", padding: "0.55rem 0.75rem", borderRadius: "6px", border: "1px solid #CBD5E1", fontSize: "0.9rem", outline: "none" }} />
              </div>
              <div style={{ display: "flex", gap: "0.5rem", marginTop: "0.5rem" }}>
