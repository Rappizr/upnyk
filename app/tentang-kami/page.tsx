  ...
  return (
    <div className="pn-root" style={{ minHeight: "100vh", background: "var(--paper)", fontFamily: "var(--font-sans), system-ui, sans-serif", color: "var(--ink)", overflowX: "hidden" }}>
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .glass-nav {
          background: ${isScrolled ? 'rgba(246, 247, 244, 0.92)' : 'rgba(255, 255, 255, 0.82)'};
          backdrop-filter: blur(18px);
          -webkit-backdrop-filter: blur(18px);
          border-bottom: 1px solid ${isScrolled ? 'rgba(12,31,23,0.08)' : 'transparent'};
        ...
        .modern-card:hover {
          ...
          box-shadow: 0 25px 50px -12px rgba(10,32,24,0.12);
        ...
        .gallery-card {
          ...
          border: 1px solid rgba(255,255,255,0.12); box-shadow: 0 18px 40px -22px rgba(0,0,0,0.3);
          transition: transform 0.4s cubic-bezier(0.4,0,0.2,1), box-shadow 0.4s ease;
        }
        .gallery-card::after {
          content: ""; position: absolute; inset: 0;
          background: linear-gradient(180deg, transparent 55%, rgba(10,32,24,0.65));
          opacity: 0; transition: opacity 0.4s ease;
        }
        .gallery-card img { width: 100%; height: 100%; object-fit: cover; transition: transform 0.6s cubic-bezier(0.4,0,0.2,1); }
        .gallery-card:hover { transform: translateY(-6px); box-shadow: 0 30px 60px -24px rgba(0,0,0,0.4); }
      ...
      <header className="glass-nav header-container" style={{ padding: "0.9rem 4rem", display: "flex", alignItems: "center", justifyContent: "space-between", position: "fixed", top: 0, left: 0, width: "100%", zIndex: 999, transition: "all 0.4s ease", boxSizing: "border-box" }}>
        ...
        <nav style={{ display: "flex", alignItems: "center" }}>
          <Link href="/" className="btn-back" style={{
            ...
            backgroundColor: "var(--emerald)", color: "#FFFFFF", borderRadius: "99px", textDecoration: "none", transition: "all 0.3s ease",
            boxShadow: "0 4px 14px rgba(18,134,78,0.25)"
          }}>
            <ArrowLeft size={16} />
            Kembali ke Beranda
          </Link>
      ...
      <section className="hero-section" style={{
        ...
        <div className="fade-in" style={{ maxWidth: "860px", margin: "0 auto", position: "relative", zIndex: 1, display: "flex", flexDirection: "column", alignItems: "center" }}>
          ...
          <div className="hero-badge" style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", padding: "0.5rem 1.3rem", borderRadius: "99px", background: "rgba(18,134,78,0.08)", border: "1px solid rgba(18,134,78,0.25)", color: "var(--emerald)", fontWeight: 800, fontSize: "0.78rem", letterSpacing: "0.1em", marginBottom: "1.6rem", textTransform: "uppercase" }}>
            <ShieldCheck size={16} color="var(--emerald)" />
            Infrastruktur Kepercayaan Rantai Pasok
          </div>

          <h1 className="hero-title" style={{ fontSize: "3.6rem", fontWeight: 800, color: "var(--ink)", lineHeight: 1.15, marginBottom: "1.4rem", letterSpacing: "-0.03em" }}>
            Memutus Rantai Tengkulak,<br />Membangun <span className="grad-text">Harga yang Adil</span>
          </h1>

          <p className="hero-desc" style={{ fontSize: "1.15rem", color: "var(--muted)", lineHeight: 1.7, fontWeight: 400, maxWidth: "720px", margin: "0 auto" }}>
        ...
        <div className="problem-grid-main" style={{ display: "grid", gridTemplateColumns: "1fr 1.2fr", gap: "4rem", alignItems: "center" }}>
          ...
          <div className="problem-grid-cards" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.25rem" }}>
            {masalah.map((m, i) => {
              const Icon = m.icon;
              return (
                <div key={i} className="problem-mini-card modern-card" style={{ padding: "1.5rem", borderRadius: "1rem" }}>
