...
export default function ProfilUMKM({ profil, setProfil }: Props) {
    ...
    <main style={{ padding: "2rem", maxWidth: "640px" }}>
      ...
      {!editing ? (
        <div style={{ background: "white", borderRadius: "14px", border: "1px solid #E2E8F0", overflow: "hidden" }}>
          {/* Hero cover */}
          <div style={{ background: "linear-gradient(135deg, #2563EB, #1D4ED8)", height: "110px", position: "relative" }} />
          <div style={{ padding: "0 1.5rem 1.5rem", marginTop: "-52px" }}>
            <div style={{ display: "flex", alignItems: "flex-end", gap: "1rem", flexWrap: "wrap" }}>
              <div style={{ position: "relative", flexShrink: 0 }}>
                <div style={{ width: "100px", height: "100px", borderRadius: "50%", background: "#EFF6FF", color: "#2563EB", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "2rem", fontWeight: 700, border: "4px solid white", overflow: "hidden" }}>
                  {profil.fotoUrl ? <img src={profil.fotoUrl} alt="Foto profil" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : profil.inisial}
                </div>
                <button onClick={() => fileRef.current?.click()} style={{ position: "absolute", bottom: 0, right: 0, width: "30px", height: "30px", borderRadius: "50%", background: "#2563EB", color: "white", border: "3px solid white", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }} aria-label="Ubah foto profil">
                  <IconCamera />
                </button>
                <input ref={fileRef} type="file" accept="image/*" onChange={handleFotoChange} style={{ display: "none" }} />
              </div>
              <div style={{ paddingBottom: "0.5rem" }}>
                <div style={{ fontSize: "1.25rem", fontWeight: 700, color: "#1E293B" }}>{profil.nama}</div>
                <div style={{ fontSize: "0.9rem", color: "#64748B" }}>{profil.usaha}</div>
                {profil.terverifikasi && (
                  <span style={{ display: "inline-flex", alignItems: "center", gap: "5px", background: "#D1FAE5", color: "#065F46", fontSize: "0.72rem", fontWeight: 600, padding: "0.25rem 0.6rem", borderRadius: "999px", marginTop: "0.4rem" }}><IconCheckCircle /> Terverifikasi</span>
                )}
              </div>
            </div>
          </div>
          <div style={{ padding: "0 1.5rem 1.5rem", display: "flex", flexDirection: "column", gap: "1rem" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "0.7rem" }}>
              <span style={{ color: "#2563EB", background: "#EFF6FF", width: "34px", height: "34px", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center" }}><IconMapPin /></span>
              <div><div style={{ fontSize: "0.7rem", color: "#94A3B8", textTransform: "uppercase", fontWeight: 700 }}>Alamat Usaha</div><div style={{ fontSize: "0.9rem", color: "#1E293B" }}>{profil.alamat}</div></div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: "0.7rem" }}>
              <span style={{ color: "#2563EB", background: "#EFF6FF", width: "34px", height: "34px", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center" }}><IconPhone /></span>
              <div><div style={{ fontSize: "0.7rem", color: "#94A3B8", textTransform: "uppercase", fontWeight: 700 }}>Telepon</div><div style={{ fontSize: "0.9rem", color: "#1E293B" }}>{profil.telepon}</div></div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: "0.7rem" }}>
              <span style={{ color: "#2563EB", background: "#EFF6FF", width: "34px", height: "34px", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center" }}><IconMail /></span>
              <div><div style={{ fontSize: "0.7rem", color: "#94A3B8", textTransform: "uppercase", fontWeight: 700 }}>Email</div><div style={{ fontSize: "0.9rem", color: "#1E293B" }}>{profil.email}</div></div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: "0.7rem" }}>
              <span style={{ color: "#2563EB", background: "#EFF6FF", width: "34px", height: "34px", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center" }}><IconUser /></span>
              <div><div style={{ fontSize: "0.7rem", color: "#94A3B8", textTransform: "uppercase", fontWeight: 700 }}>Kategori Usaha</div><div style={{ fontSize: "0.9rem", color: "#1E293B" }}>{profil.kategori}</div></div>
            </div>
            <button onClick={() => { setForm(profil); setEditing(true); }} style={{ marginTop: "0.5rem", padding: "0.65rem", borderRadius: "8px", border: "none", background: "#2563EB", color: "white", fontWeight: 600, cursor: "pointer" }}>Edit Profil</button>
          </div>
        </div>
      ) : (
        <form onSubmit={handleSubmit} style={{ background: "white", borderRadius: "14px", border: "1px solid #E2E8F0", padding: "1.5rem", display: "flex", flexDirection: "column", gap: "0.9rem" }}>
          ...
          <div>
            ...
            <input required value={form.usaha} onChange={(e) => setForm({ ...form, usaha: e.target.value })} style={{ width: "100%", padding: "0.55rem 0.75rem", borderRadius: "8px", border: "1px solid #CBD5E1", fontSize: "0.9rem", outline: "none" }} />
          </div>
          <div>
          ...
          <div>
            ...
            <input value={form.kategori} onChange={(e) => setForm({ ...form, kategori: e.target.value })} style={{ width: "100%", padding: "0.55rem 0.75rem", borderRadius: "8px", border: "1px solid #CBD5E1", fontSize: "0.9rem", outline: "none" }} />
          </div>
          <div style={{ display: "flex", gap: "0.5rem", marginTop: "0.5rem" }}>
            ...
            <button type="submit" style={{ flex: 1, padding: "0.6rem", borderRadius: "8px", border: "none", background: "#2563EB", color: "white", fontWeight: 600, cursor: "pointer" }}>Simpan Perubahan</button>
          </div>
        </form>
      )}
    </main>
